...
function parse(files, options, callback) {
  ...
  function parseFiles (files) {
    ...
    files.forEach(function (file) {
      ...
      strings = parser.parse(template, ctxSeparator);

      for (var msgid in strings) {
        if (strings.hasOwnProperty(msgid)) {
          context[msgid] = context[msgid] || {msgid: strings[msgid].msgid || msgid, comments: {}};
          ...
          if (strings[msgid].ctx) {
            context[msgid].msgctxt = context[msgid].msgctxt || strings[msgid].ctx;
          }
